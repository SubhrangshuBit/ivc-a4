...
class CamVidDataset(Dataset):
    ...
    def parse_class_dict(self, class_dict_path):
        ...
        return class_dict.apply(lambda x: ((x.r, x.g, x.b), x.values[0]), axis=1).to_dict()
    ...
    def rgb_to_class_id(self, label_img):
        ...
        label_imgarr = np.array(label_img)

        # Mask to match RGB values to class labels
        label_mask = np.zeros(label_imgarr.shape[:2], dtype=np.uint8)

        for label, map in self.class_dict.items():
            rgb = np.array(map[0])
            # Match along the last axis for each pixel
            matches = np.all(label_imgarr == rgb, axis=-1)
            # Set corresponding mask to class id
